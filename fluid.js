/* ==========================================================================
   Fluid-ink WebGL background (Pavel Dobryakov's WebGL fluid sim, MIT),
   lifted from the Flowstate hero spec and reused here as the portfolio's
   hero background. Config below is tuned down slightly for a subtler,
   more "ambient" look behind text-heavy content rather than the bright
   full-immersion original.
   ========================================================================== */
...
function fluidSimulation(canvas) {
    ...
    initFramebuffers();
    // Entrance burst plus a few rapid follow-up waves (drained one-per-frame by input()).
    multipleSplats(24);
    for (let i = 0; i < 5; i++)
...
const ORBIT_START_DELAY = 700;
    ...
    // An invisible auto-cursor orbits the centre, keeping the background alive
    // without requiring the visitor to move their mouse over the hero.
    function driveVirtualPointer() {
        if (Date.now() - engineStart < ORBIT_START_DELAY)
            return;
        const cx = canvas.width / 2;
        const cy = canvas.height / 2;
        const base = Math.min(ORBIT_RADIUS, canvas.width * 0.4, canvas.height * 0.55);
        ...
        if (!virtualColor || Date.now() - lastVColorTime > 160) {
        ...
        const dx = (x - vPrevX) * 7.0;
        const dy = (y - vPrevY) * 7.0;
        vPrevX = x;
        vPrevY = y;
    ...
    // The canvas sits behind page content (pointer-events: none), so pointer
    // data is read from window events and mapped into canvas space via its
    // bounding rect. Listeners are passive and never preventDefault, so page
    // scroll is untouched.
    ...
    function generateColor() {
        // Cyan -> blue -> violet band, tuned to sit behind the portfolio's
        // teal/blue accent palette rather than the wider cyan-magenta spread.
        const h = 0.5 + Math.random() * 0.16;
